    ...
    return (
        <section className="w-full h-full flex flex-col lg:flex-row items-center justify-center px-4 md:px-[80px] py-[100px] gap-16 overflow-hidden">
            ...
            >
                ...
                >
                    <h3 className="text-4xl md:text-5xl font-extrabold mb-1">{aboutExpertise.badgeValue}</h3>
                    <p className="text-sm md:text-base font-semibold leading-tight">{aboutExpertise.badgeLabelPart1}<br/>{aboutExpertise.badgeLabelPart2}</p>
                </motion.div>
            </motion.div>
            ...
            <div className="w-full lg:w-[55%] flex flex-col gap-10 mt-10 lg:mt-0 lg:pl-10">
                ...
                >
                    <h2 className="text-white text-[32px] md:text-[45px] lg:text-[50px] font-bold leading-[1.2] tracking-tight">
                        {aboutExpertise.title}<br className="hidden md:block" />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent-secondary to-accent">{aboutExpertise.titleSpan}</span>
                    </h2>
                    <p className="text-white/70 text-[18px] leading-relaxed">
                        {aboutExpertise.description}
                    </p>
                </motion.div>

                {/* Progress Bars Grid */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {aboutExpertise.expertise.map((item, index) => (
                        <motion.div
                            key={index}
                            initial={{ opacity: 0, y: 30 }}
                            whileInView={{ opacity: 1, y: 0 }}
